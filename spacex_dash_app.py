...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        total_success_failure = spacex_df['Launch Site'].value_counts()
        fig = px.pie(
        values=total_success_failure, 
        names=total_success_failure.index, 
        title='Success/Failure for All Launch Sites')
        return fig
    else:
...
def render_scatter_chart(selected_site, selected_payload_range):
    ...
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                     ...
                     color_discrete_map={'0': 'red', '1': 'green'})  # Custom color mapping for Launch Outcome
